...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    console.log('📄 File1 rows:', file1Data?.length || 0);
    console.log('📄 File2 rows:', file2Data?.length || 0);

    if (!script || !file1Data || !file2Data) {
      throw new Error('Missing required parameters');
    }

    // Execute the EXACT same logic as standardReconciliation.js
    const result = standardReconciliation(file1Data, file2Data);

    console.log('✅ Script completed, result rows:', result?.length || 0);
...
// EXACT COPY of standardReconciliation logic from scripts/standardReconciliation.js
function standardReconciliation(file1Data, file2Data) {
    ...
    if (file1Data.length < 2 || file2Data.length < 2) {
        throw new Error('Insufficient data in files');
    }

    // EXACT column mapping from standardReconciliation.js
    ...
    // Process file1 with EXACT same logic
    ...
    // Process file2 (EXACT same logic)
    let jsonData2 = [];
    let file2Headers = file2Data[0] || [];
    let dateClosedIndex = file2Headers.findIndex(header => 
        typeof header === "string" && header.trim().toLowerCase() === "date closed"
    );
    let nameIndex = file2Headers.findIndex(header => 
        typeof header === "string" && header.trim().toLowerCase() === "name"
    );
    let amountIndex = file2Headers.findIndex(header => 
    ...
    jsonData1.forEach(row => {
        ...
        // Calculate K-R value (EXACT same formula)
        ...
        // Calculate Count - matches in second file (EXACT same logic)
        let countMatches = 0;
    ...
    // Calculate Final Count for First File Rows (EXACT same logic)
    if (file2Data && file2Headers.length > 0) {
    ...
    // Only add rows with Final Count = 0 (EXACT same filtering)
    ...
    // Card brand totals calculation (EXACT same logic)
    ...
    // Process second file for name totals (EXACT same logic)
    const nameTotals = {};
    
    if (file2Data && file2Headers.length > 0 && nameIndex !== -1 && amountIndex !== -1) {
        const commonNames = {
            "visa": "Visa",
    ...
    // Add card brand comparison rows (EXACT same order and logic)
    ...
    console.log('✅ Reconciliation completed with admin-matching logic');
